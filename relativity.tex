  ...
  \begin{center}
    \begin{tikzpicture}
        ...
        \visible<6,7>{\node at (-2,-1.4){$t_c$};  
          ...
          %\visible<4>{\node[text width=4cm] at (0,-1.5){Synchronize if:\\$t_b-t_a=t_c-t_b$};}}
            \visible<8,9>{
              \def\x{3}
              \def\h{0}
              \def\xn{-2}
            ...
            \visible<9,10>{\path[light] ($(midtrain)+(3mm,1mm)$)--($(midtrain)+(25.5mm,1mm)$);}
            \visible<9,10>{\path[light] ($(midtrain)+(-3mm,1mm)$)--($(midtrain)-(25.5mm,-1mm)$);}
              %\draw[ultra thick] (1,0.0) circle (0.3cm);
              %\draw[ultra thick] (4,0.0) circle (0.3cm);
            \visible<10->{
              \def\x{4}
              \def\h{0}
              \def\xn{-1}
...
\begin{frame}{Gravity and light}
  \begin{columns}
    \begin{column}{0.5\textwidth}
     \begin{itemize}
       \item{The equivalence principle predicts that light is being bent by gravitational fields}
 ...
 \begin{tikzpicture}
        ...
        \only<2>{\draw[ultra thick,yellow] (0.75,0.75) parabola (1.8,0.1);
...
\begin{frame}{Solar eclipse}
  ...
  \begin{tikzpicture}
    ...
    \node at (2,0){\includegraphics[width=1cm]{fig/sun.png}};
  \end{tikzpicture}
